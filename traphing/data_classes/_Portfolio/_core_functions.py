...
def add_symbol(self, symbol_name: str, timeframes_list: List[Timeframes] = None, symbol: Symbol = None):
    if symbol is None:
        symbol = Symbol(symbol_name, timeframes_list);
    elif timeframes_list is not None:
        symbol = Symbol.get_subsymbol(timeframes_list)
    else: 
        symbol = symbol
...
def map_timeframe(self, timeframe, method_name, *args, **kwargs):
    ...
    forex_intraday_close_values_pd = pd.concat([getattr(self[symbol_name][timeframe], method_name)(*args, **kwargs) for symbol_name in self.symbol_names_list],
                                                axis = 1, keys = self.symbol_names_list)
...
def get_currencies_with_market_hours(self, open_time = dt.time(0), close_time = dt.time(0)):
    """It returns the symbol_names_list of the symbols which have the given
...
def get_non_empty(self, timeframe):
    """ Ir returns the symbol_names_list without the symbol_name of the symbols
