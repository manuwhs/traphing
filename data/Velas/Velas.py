...
#### IMPORT the methods
import velas_core as vc    # Core methods
import velas_graph as vgr   # Graphics
import velas_DB as vDB   #
import velas_indicators as vind
import velas_indicators_pandas as vindpd
...
class Velas:
    ...
    def __init__(self, symbol_name: str = None, timeframe: int = None,  TD: pd.DataFrame = ul.empty_df):
        ...
        self.set_(TD)   # To make the interval
    ...
    def dates(self):
        if (len(indexes) == 0):
            indexes = self._time_mask
        return self._df.index[indexes]

    @dates.setter
    def dates(self, value: pd.TimeIndex ):
    ...
    ############## DDBB methods ###########################################
    #######################################################################
    set_csv = TDDB.set_    # Set and add timeData from csv's
