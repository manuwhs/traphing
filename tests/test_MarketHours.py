...
from traphing.utils import MarketHours, SpecialMarketHours, Timeframes
...
def get_loaded_market_hours():
    ...
    special_market_hours = SpecialMarketHours(
        special_date, open_time=dt.time(9, 0, 0), close_time=dt.time(14, 0, 0))
    special_days_dict = {special_market_hours.date: special_market_hours}
    my_market_hours = MarketHours(
        open_time, close_time, trading_days_list, special_days_dict=special_days_dict)
...
class TestMarketHours():
    ...
    def test_setting_special_days_dict(self):
        special_date = dt.date(year=2019, month=12, day=25)
        special_market_hours = SpecialMarketHours(
            special_date, open_time=dt.time(9, 0, 0))
        special_days_dict = {special_market_hours.date: special_market_hours}
    ...
    def test_datetime_related_chekings(self):
        ...
        special_datetime = dt.datetime(
            special_date.year, special_date.month, special_date.day, 15, 0, 0)
    ...
    def test_other_trading_session_info(self):
        ...
        assert my_market_hours.get_length_session_in_seconds(
            datetime_1.date()) == (28800, 57600)
        assert my_market_hours.get_length_session_in_seconds(
            special_date) == (18000, 68400)
        assert my_market_hours.get_number_of_samples_of_trading_session(
            Timeframes.M15, datetime_1.date()) == 32
        assert my_market_hours.get_number_of_samples_of_trading_session(
            Timeframes.M15, special_date) == 20

    def test_estimate_timeframe(self):
        my_market_hours = get_loaded_market_hours()
        my_velas_M15 = get_loaded_Velas()
        timestamps_M15 = my_velas_M15.timestamps
        assert my_market_hours.estimate_timeframe(
            timestamps_M15) == Timeframes.M15
    ...
    def test_estimate_open_close_time(self):
        ...
        assert my_market_hours.estimate_open_close_time(
            timestamps_M15) == (time_0, time_0)
    ...
    def test_estimate_normal_trading_days(self):
        ...
        timestamps_M15 = my_velas_M15.timestamps
        days_list = [0, 1, 2, 3, 4]
        assert my_market_hours.estimate_normal_trading_days(
            timestamps_M15) == days_list
    ...
    def test_estimate_special_trading_days_from_timestamps(self):
        ...
        special_days_dict = my_market_hours.estimate_special_trading_days_from_timestamps(
            timestamps_M15)
    ...
    def test_index_by_days_dict(self):
        ...
        my_market_hours.estimate_special_trading_days_from_timestamps(
            timestamps_M15)
        index_by_days_dict = MarketHours.get_index_by_days_dict(timestamps_M15)
        number_of_samples_per_trading_session = my_market_hours.get_number_of_samples_of_trading_session(
            my_velas_M15.timeframe)
    ...
    def test_number_of_samples_by_weekday_dict(self):
        ...
        samples_by_weekday_dict = my_market_hours.get_number_of_samples_by_weekday_dict(
            timestamps_M15)
